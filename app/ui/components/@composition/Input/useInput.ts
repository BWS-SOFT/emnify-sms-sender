...
export const useInput = () => {
	const { showError, possitionIcon, useAddIcon } =
	...
	const inputClass = tv({
		base: `block w-full rounded-md border-0 px-3 py-1.5 
		...
		variants: {
			side: {
				left: `rounded-none ${
					useAddIcon ? 'rounded-r-md' : 'rounded-md'
				}`,
				'left-iconed': 'pl-10',
				right: `rounded-none ${
					useAddIcon ? 'rounded-l-md' : 'rounded-md'
				}`,
				'right-iconed': 'pr-10',
			} as ClassNameType,
		},
	...
	const defaultAddOn = tv({
		base: `inline-flex items-center bg-gray-200 border border-gray-300 px-3 
			text-gray-500 sm:text-sm     dark:border-gray-600`,
		variants: {
			side: {
